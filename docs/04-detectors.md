...
| **electron-updaters** | none | `~/Library/Caches/*-updater`, `~/Library/Caches/*.ShipIt` (Squirrel.Mac). Verified ~2 GB on the planning machine: Lens 1.07 GB, Bitwarden 348 MB, Notion 297 MB + 242 MB | regenerable; attributed to the app by name/bundle id when possible |
| **other ecosystems** | none | Flutter/Dart `~/.pub-cache`, `~/flutter/bin/cache`; .NET `~/.nuget/packages`, `~/.dotnet`; Haskell `~/.ghcup`, `~/.stack`, `~/.cabal`; Bazel `/private/var/tmp/_bazel_<user>`; `~/.ccache`; `~/.conan2`; SwiftPM `~/Library/Caches/org.swift.swiftpm`; `~/.cocoapods/repos`; Elixir `~/.mix`, `~/.hex`; PHP `~/.composer/cache`; Zig `~/.cache/zig` | caches: regenerable; SDKs: tool-managed |
...
|---|---|---|---|
| **orbstack** | `orb list` (verified: prints `[]` when no Linux machines), `docker context ls` (verified: context named `orbstack` with socket `~/.orbstack/run/docker.sock`), `docker system df --format json` (verified: one JSON object per line with **human-formatted strings** like `"Size":"9.853GB"`, `"Reclaimable":"5.379GB (54%)"`; parse units), `docker system df -v` for per-image detail | Disk image at `~/Library/Group Containers/HUAQ24HBR6.dev.orbstack/data/data.img.raw` (verified sparse: 245 GB apparent / 18.8 GB allocated) plus `swap.img`; `~/.orbstack` is config/run/log only; `~/Library/Caches/dev.orbstack*` | On the planning machine the daemon reports ~20 GB across images/containers/volumes/build cache vs 18.8 GB allocated on host, i.e. auto-shrink is working; still show both numbers |
| **docker** (Desktop) | `docker context ls`, `docker system df --format json` per context (verified on the OrbStack context); `-v` combined with `--format` is historically unsupported in the Docker CLI, so run `docker system df -v` unformatted and parse the table when per-image detail is wanted. Docker Desktop itself is **not installed on the planning machine**, so this detector is written from documentation and validated later | `~/Library/Containers/com.docker.docker/Data/vms/0/data/Docker.raw` (sparse), `~/.docker` (config, buildx, scout cache) | If both OrbStack and Desktop exist, attribute by docker context; images/containers/volumes/build-cache split from `system df` |
